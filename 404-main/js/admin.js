...
// Recent bookings status filter state
if (!adminState.recentStatusFilter) {
  adminState.recentStatusFilter = 'all';
}
...
function renderRecentBookings(bookings) {
  ...
  // Count by status for badges (before filtering)
  ...
  const cancelledCount = bookings.filter(b => {
    const s = (b.status || '').toLowerCase();
    return s === 'cancelled' || s === 'cancelledbycustomer' || s === 'cancelledbyadmin' || s === 'cancelledbysystem';
  }).length;

  // Apply status filter
  const statusFilter = adminState.recentStatusFilter || 'all';
  ...
  if (statusFilter !== 'all') {
    filteredBookings = bookings.filter(b => {
      ...
      if (statusFilter === 'in-progress') return status === 'in-progress' || status === 'inprogress' || status === 'in progress';
      if (statusFilter === 'completed') return status === 'completed';
      if (statusFilter === 'cancelled') return status === 'cancelled' || status === 'cancelledbycustomer' || status === 'cancelledbyadmin' || status === 'cancelledbysystem';
      return true;
    });
  }
  ...
  if (adminState.recentLimit !== 'all') {
    ...
    displayBookings = filteredBookings.slice(0, limitNum);
  }

  // Build filter tabs HTML - mobile responsive with CSS Grid
  const filterTabsHtml = `
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(90px, 1fr)); gap: 0.5rem; margin-bottom: 1rem;">
      <button class="btn btn-sm ${statusFilter === 'all' ? 'btn-primary' : 'btn-outline'}" onclick="filterRecentBookingsByStatus('all')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem;">
        📋 All <span style="background: ${statusFilter === 'all' ? 'rgba(255,255,255,0.3)' : 'var(--gray-200)'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${allCount}</span>
      </button>
      <button class="btn btn-sm ${statusFilter === 'pending' ? 'btn-primary' : 'btn-outline'}" onclick="filterRecentBookingsByStatus('pending')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem; ${statusFilter === 'pending' ? '' : 'color: #f59e0b; border-color: #f59e0b;'}">
        ⏳ Pending <span style="background: ${statusFilter === 'pending' ? 'rgba(255,255,255,0.3)' : '#fef3c7'}; color: ${statusFilter === 'pending' ? 'white' : '#f59e0b'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${pendingCount}</span>
      </button>
      <button class="btn btn-sm ${statusFilter === 'confirmed' ? 'btn-primary' : 'btn-outline'}" onclick="filterRecentBookingsByStatus('confirmed')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem; ${statusFilter === 'confirmed' ? '' : 'color: #10b981; border-color: #10b981;'}">
        ✅ Confirmed <span style="background: ${statusFilter === 'confirmed' ? 'rgba(255,255,255,0.3)' : '#d1fae5'}; color: ${statusFilter === 'confirmed' ? 'white' : '#10b981'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${confirmedCount}</span>
      </button>
      <button class="btn btn-sm ${statusFilter === 'in-progress' ? 'btn-primary' : 'btn-outline'}" onclick="filterRecentBookingsByStatus('in-progress')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem; ${statusFilter === 'in-progress' ? '' : 'color: #0d6efd; border-color: #0d6efd;'}">
        🔄 Active <span style="background: ${statusFilter === 'in-progress' ? 'rgba(255,255,255,0.3)' : '#cfe2ff'}; color: ${statusFilter === 'in-progress' ? 'white' : '#0d6efd'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${inProgressCount}</span>
      </button>
      <button class="btn btn-sm ${statusFilter === 'completed' ? 'btn-primary' : 'btn-outline'}" onclick="filterRecentBookingsByStatus('completed')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem; ${statusFilter === 'completed' ? '' : 'color: #2e7d32; border-color: #2e7d32;'}">
        🎉 Done <span style="background: ${statusFilter === 'completed' ? 'rgba(255,255,255,0.3)' : '#e8f5e9'}; color: ${statusFilter === 'completed' ? 'white' : '#2e7d32'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${completedCount}</span>
      </button>
      <button class="btn btn-sm ${statusFilter === 'cancelled' ? 'btn-primary' : 'btn-outline'}" onclick="filterRecentBookingsByStatus('cancelled')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem; ${statusFilter === 'cancelled' ? '' : 'color: #dc3545; border-color: #dc3545;'}">
        ❌ Cancelled <span style="background: ${statusFilter === 'cancelled' ? 'rgba(255,255,255,0.3)' : '#f8d7da'}; color: ${statusFilter === 'cancelled' ? 'white' : '#dc3545'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${cancelledCount}</span>
      </button>
    </div>
  `;
  ...
  container.innerHTML = filterTabsHtml + currentSlice.map(booking => {
    ...
    const statusClass = ['confirmed', 'completed'].includes(statusLower)
      ? 'badge-confirmed'
      : statusLower === 'in progress' || statusLower === 'in-progress' || statusLower === 'inprogress'
        ? 'badge-inprogress'
...
window.filterRecentBookingsByStatus = function(status) {
  adminState.recentStatusFilter = status;
  adminState.recentPage = 1;
  renderRecentBookings(adminState.recentData);
};
...
window.loadBookingHistory = async function () {
  ...
  const historyBookings = bookings.filter(b => {
    ...
    if (status === 'no-show' || status === 'noshow') return true;
