  ...
  try {
    ...
    if (user) {
      try {
        ...
        // Check if editing existing booking - skip profile loading
...
async function submitBooking(event) {
  ...
  try {
    ...
    if (isDuplicate) {
      ...
      await customAlert.warning('Duplicate Booking', `You already have a pending booking for "${booking.petName}" on ${formatDate(booking.date)} at ${formatTime(booking.time)}.\n\nPlease select a different date or time.`);
    ...
    if (typeof createBookingWithValidation === 'function') {
      ...
      if (!createdBooking || !createdBooking.id) {
        console.error('[submitBooking] createBookingWithValidation returned invalid booking:', createdBooking);
      ...
      // Verify booking was created
      if (!createdBooking || !createdBooking.id) {
        console.error('[submitBooking] createBookingSecure returned invalid booking:', createdBooking);
        throw new Error('Booking creation returned invalid data');
      }
    ...
    try {
      ...
      console.warn('[submitBooking] Log history error (non-critical):', logHistoryErr);
    }
    ...
    try {
      ...
      console.warn('[submitBooking] SessionStorage error (non-critical):', storageErr);
    }
    ...
    if (submitBtn) {
      try {
        ...
        console.warn('[submitBooking] Cooldown error (non-critical):', cooldownErr);
      }
    }
    ...
    if (typeof hideLoadingOverlay === 'function') {
      try {
        ...
        console.warn('[submitBooking] Hide overlay error (non-critical):', hideErr);
      }
    }
    ...
    try {
      ...
      console.warn('[submitBooking] Status update error (non-critical):', statusErr);
    }
    ...
    try {
      ...
      console.warn('[submitBooking] Logging error (non-critical):', logErr);
    }
    ...
    // Use direct window.location.href for reliability
    // ============================================
    console.log('[submitBooking] Booking created successfully! Redirecting to booking-success.html...');
    console.log('[submitBooking] Booking ID:', booking.id);
    
    // Direct redirect without setTimeout for reliability
    ...
    // For network timeouts, the booking might have been created
    // but the response didn't come back
    // ============================================
    let bookingWasCreated = false;
    try {
      if (typeof booking !== 'undefined' && booking.petName && booking.date && booking.time) {
        console.log('[submitBooking] Checking if booking was created despite error...');
        const existingBookings = await getBookings();
        const possibleBooking = existingBookings.find(b => 
          b.userId === user?.id &&
          b.date === booking.date &&
          b.time === booking.time &&
          b.petName === booking.petName &&
          ...
          (Date.now() - (b.createdAt || 0)) < (2 * 60 * 1000)
        );
        
        if (possibleBooking) {
          console.log('[submitBooking] Booking WAS created despite error! ID:', possibleBooking.id);
          bookingWasCreated = true;
          
          // Save to session and redirect to success
          sessionStorage.setItem('lastBooking', JSON.stringify(possibleBooking));
          ...
          if (typeof customAlert !== 'undefined' && customAlert.success) {
            await customAlert.success('Booking Created', 'Your booking was created successfully! Redirecting...');
          }
      ...
      console.warn('[submitBooking] Could not check if booking was created:', checkError);
    }
    ...
    if (err.message) {
      if (err.message.includes('time slot') || err.message.includes('no longer available')) {
        ...
        errorMessage = 'This booking may already exist. Please check your bookings in your dashboard.';
        ...
        errorMessage = 'You do not have permission to create this booking. Please try logging in again.';
    ...
    console.error('[submitBooking] Error details:', errorMessage);
    
    // Use customAlert for better UX
    if (typeof customAlert !== 'undefined' && customAlert.error) {
      await customAlert.error(errorTitle, errorMessage);
    } else {
