...
async function createBookingSecure(bookingData) {
  ...
  try {
    ...
    // Find the most recently created booking for this user
    const allBookings = result.snapshot.val() || {};
    let createdBooking = null;
    let latestCreatedAt = 0;
    
    for (const [id, b] of Object.entries(allBookings)) {
      if (b.userId === user.id && 
          b.date === bookingData.date && 
          b.time === bookingData.time &&
          b.petName === bookingData.petName &&
          b.createdAt > latestCreatedAt) {
