...
async function renderCustomerBookings() {
  ...
  const allCount = customerBookingsCache.length;
  const cancelledStatuses = ['cancelled', 'cancelledByCustomer', 'cancelledByAdmin', 'cancelledBySystem'];
  const pendingCount = customerBookingsCache.filter(b => normalizeStatus(b.status) === 'pending').length;
  const confirmedCount = customerBookingsCache.filter(b => normalizeStatus(b.status) === 'confirmed' || isInProgressStatus(b.status)).length;
  const completedCount = customerBookingsCache.filter(b => normalizeStatus(b.status) === 'completed').length;
  const cancelledCount = customerBookingsCache.filter(b => cancelledStatuses.includes(b.status)).length;

  // Get current filter
  const currentFilter = customerBookingState.filter || 'all';

  // Build filter tabs HTML - mobile responsive
  const filterTabsHtml = `
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(100px, 1fr)); gap: 0.5rem; margin-bottom: 1rem;">
      <button class="btn btn-sm ${currentFilter === 'all' ? 'btn-primary' : 'btn-outline'}" onclick="setCustomerBookingFilter('all')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem;">
        📋 All <span style="background: ${currentFilter === 'all' ? 'rgba(255,255,255,0.3)' : 'var(--gray-200)'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${allCount}</span>
      </button>
      <button class="btn btn-sm ${currentFilter === 'pending' ? 'btn-primary' : 'btn-outline'}" onclick="setCustomerBookingFilter('pending')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem; ${currentFilter === 'pending' ? '' : 'color: #f59e0b; border-color: #f59e0b;'}">
        ⏳ Pending <span style="background: ${currentFilter === 'pending' ? 'rgba(255,255,255,0.3)' : '#fef3c7'}; color: ${currentFilter === 'pending' ? 'white' : '#f59e0b'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${pendingCount}</span>
      </button>
      <button class="btn btn-sm ${currentFilter === 'confirmed' ? 'btn-primary' : 'btn-outline'}" onclick="setCustomerBookingFilter('confirmed')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem; ${currentFilter === 'confirmed' ? '' : 'color: #10b981; border-color: #10b981;'}">
        ✅ Confirmed <span style="background: ${currentFilter === 'confirmed' ? 'rgba(255,255,255,0.3)' : '#d1fae5'}; color: ${currentFilter === 'confirmed' ? 'white' : '#10b981'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${confirmedCount}</span>
      </button>
      <button class="btn btn-sm ${currentFilter === 'completed' ? 'btn-primary' : 'btn-outline'}" onclick="setCustomerBookingFilter('completed')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem; ${currentFilter === 'completed' ? '' : 'color: #2e7d32; border-color: #2e7d32;'}">
        🎉 Done <span style="background: ${currentFilter === 'completed' ? 'rgba(255,255,255,0.3)' : '#e8f5e9'}; color: ${currentFilter === 'completed' ? 'white' : '#2e7d32'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${completedCount}</span>
      </button>
      <button class="btn btn-sm ${currentFilter === 'cancelled' ? 'btn-primary' : 'btn-outline'}" onclick="setCustomerBookingFilter('cancelled')" style="display: flex; align-items: center; justify-content: center; gap: 0.3rem; padding: 0.5rem; font-size: 0.8rem; ${currentFilter === 'cancelled' ? '' : 'color: #dc3545; border-color: #dc3545;'}">
        ❌ Cancelled <span style="background: ${currentFilter === 'cancelled' ? 'rgba(255,255,255,0.3)' : '#f8d7da'}; color: ${currentFilter === 'cancelled' ? 'white' : '#dc3545'}; padding: 0.1rem 0.4rem; border-radius: 1rem; font-size: 0.7rem;">${cancelledCount}</span>
      </button>
    </div>
  `;

  if (!Array.isArray(customerBookingsCache) || !customerBookingsCache.length) {
    container.innerHTML = filterTabsHtml + `<div class="card fade-in" style="text-align:center; padding:3rem;"><h3>No Bookings Yet</h3></div>`;
  ...
  if (filtered.length === 0) {
    const message = currentFilter !== 'all' 
      ...
      : 'No bookings found.';
    container.innerHTML = filterTabsHtml + `<div class="card fade-in" style="text-align:center; padding:3rem;"><h3>${message}</h3></div>`;
    if (pagination) pagination.innerHTML = '';
    return;
  }
  ...
  container.innerHTML = filterTabsHtml + items.join('');
  ...
  try {
    ...
    const upcomingPendingBookings = bookings
      .filter(b => {
        ...
        // Parse booking date and time
        const bookingDateTime = new Date(b.date + ' ' + (b.time || '00:00'));
        return bookingDateTime >= now; // Must be in the future
      })
      .sort((a, b) => {
